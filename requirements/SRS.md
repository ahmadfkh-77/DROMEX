...
- Every wall section created after this feature shall have a recorded **base**, and wall geometry, layers, and wall-material consumption shall stay locked until that base is explicitly confirmed cured (DEC-459). A wall that existed beforehand shall remain fully usable, shall be shown as "Base not recorded — legacy wall", and shall never be given an invented base or curing date.
...
- Wall work shall never be dated before the cured date, a cured base shall only return to curing while no wall work exists, and a base correction that would strand existing wall work shall be refused and explained (DEC-459). Base corrections shall use the same reasoned model as wall consumption.
- The Daily Report shall include base activity on the date it happened, show the stage the base had reached **by that work date** rather than its latest stage, and never show wall work or layers on an earlier base-only report. The workbook shall carry the base geometry, volumes, quantity, override state, status, and lifecycle dates (DEC-459).
...
- Deferred to a following phase: Daily Report PDF and workbook representation of the composite breakdown, and the requested 5-stage guided-workflow screen redesign of Wall Construction as a whole (DEC-461).
