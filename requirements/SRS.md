...
### FR-005
- Statement: “The system shall maintain digital fuel records for the single stored fuel tank and for the two supported fuel types, Diesel and Gasoline.”
...
- Alternate and exception behavior: Diesel is the stored, balanced fuel; Gasoline is fill-only, has no tank balance, gauge reading or delivery, and its fills never affect the diesel balance (DEC-392). Each fuel type keeps its own current price and price history, and every fill snapshots the price it used; all fuel records predating DEC-392 are Diesel. Confirmed fills and deliveries may be corrected in place with a mandatory reason and field-level before/after audit history, recalculating ledger and financial effects; a delivery cannot be corrected to Unpriced while active payments exist, and reducing it below the amount already paid is permitted and shown as Overpaid without refund logic; cancelled fuel movements are not reactivated (DEC-393). Gauge readings remain cancel-only. Physical gauge readings reset the current baseline without rewriting prior movements. Cancelled movements remain visible and reverse their ledger effect only within the segment governed by the applicable baseline. A positive equipment fill may exceed the calculated balance without warning or blocking.
...
- Main behavior: Select a saved quarry/supplier or create one quickly, manually copy the positive whole cubic-metre quantity from the supplier ticket/invoice without requiring empty/full weighbridge entries, and record item, delivery method, automatic confirmation date/time, optional supplier ticket/invoice number, up to 20 optional ticket/invoice photos, and payment information without processing the payment. `Our Driver` requires a saved company driver and truck. `Supplier Delivering` requires neither and offers an optional supplier truck/plate; a blank plate displays as Supplier Delivering rather than an empty driver. Automatically compress photos at readable quality. Optionally accept USD price per cubic metre, calculate purchase subtotal as quantity multiplied by that unit price, apply universal VAT, and calculate final purchase total. For the current local work date, group active company deliveries by project, supplier, item, saved driver, and saved truck; group supplier deliveries by project, supplier, item, and normalized optional supplier plate, with blank plates together and different entered plates separate. Each counter shows trip count, cumulative m³, and the latest trip quantity. Pressing `+1 Trip` opens a confirmation panel prefilled with that latest quantity; the owner may change it for this trip only. Confirmation creates a new independently numbered and timestamped normal quarry purchase using the counter's project, supplier, item, delivery method/transport identity, price, VAT, and notes while leaving supplier ticket/photos empty for the new trip. Make every delivery and attachment available offline and in synchronized supplier history. Organize Purchase History as closed-by-default Supplier → Linked Project → Purchase disclosures ordered by supplier then project name, with `No project linked` last. Supplier and project labels show filtered purchase counts and active cubic-metre totals; supplier labels also show their linked-project count. Date filtering rebuilds the hierarchy and totals from matching purchases.
...
- Priority: Must
- Acceptance criteria: The owner can confirm a quantity-only quarry delivery with a positive whole number of cubic metres and without price, VAT, supplier reference, photos, or payment balance; zero, negative, and decimal quantities are rejected. Supplier Delivering confirms without a company driver or truck and with a blank or entered supplier plate. Confirming the first 20 m³ delivery today creates a one-trip counter; confirming `+1 Trip` at the unchanged value creates a second separately numbered 20 m³ purchase and shows 2 trips / 40 m³; changing the next panel to 18 creates only that trip at 18 m³ and shows 3 trips / 58 m³. Another company driver/truck or different entered supplier plate has a separate counter, blank supplier plates group together, cancelled purchases are excluded, and tomorrow requires a new first purchase. A priced purchase supports multiple order-linked payments, derived balance/status, and final payment cancellation. Confirmation records date/time; up to 20 readable-quality compressed attachments work offline/sync; the record appears in supplier history and never alters inventory. Purchase History initially shows closed supplier labels; opening one reveals only projects used with that supplier plus `No project linked` when applicable; opening a project reveals its matching purchase cards.
- Source: Interview turns 21–22, 26–30, 419, 424, and 433–434
- Status: Confirmed and implemented through Turn 434; physical Android supplier-delivery acceptance pending
...
- Priority: Must
- Acceptance criteria: The owner can add and edit a uniquely named/symbolized unit and subsequently select it. Active and Inactive groups remain organized alphabetically. Duplicate name or symbol is blocked regardless of letter case. Remove permanently deletes an unused unit; a referenced unit cannot be deleted and is instead deactivated, may be reactivated, and does not change history.
- Source: Interview turns 27–28, 46, 301, and 434
- Status: Confirmed and implemented management/lifecycle through Turn 434; physical UI acceptance pending
...
- Main behavior: Treat tank quantity as Unknown until the owner enters a physical gauge reading. Continue accepting and reporting purchases and equipment fills while quantity is unknown. After a gauge establishes the baseline, add recorded purchases, subtract equipment fills, and replace the calculated balance with a later physical gauge reading when correcting/reconciling stock. Plant consumption is excluded from movement totals.
...
- Priority: Must
- Acceptance criteria: The owner can select or quickly create equipment; confirmation is blocked unless litres are positive; the saved fill shows equipment, litres, automatic confirmation time, snapshotted price and cost or an explicit Unpriced state, and any optional supplied fields; it appears in equipment/project histories even when tank quantity is Unknown. Later price changes do not change it. A differing override requires a reason. No hour-meter or derived fuel-consumption-rate field is displayed.
...
- Main behavior: Maintain one current USD fuel price per litre with effective date/time, owner identity, optional reason, and permanent newest-first price history. Require a positive purchase litre quantity, assign confirmation time automatically, increase a known recorded fuel balance, and optionally retain supplier, ticket/invoice number, price, and notes. Default new purchases and fills to the current price. A priced confirmed purchase may establish a changed price as the new current price and historical entry. Calculate purchase subtotal, VAT snapshot, final total, and supplier balance under the established supplier-order rules.
...
- Main behavior: Fuel Dashboard separates Today and current-calendar-month purchased litres/purchase spend from filled litres/consumption cost, shows known balance or Unknown, latest measurable variance, unpriced fill litres, fuel by project, and top-consuming equipment. Balance Summary shows the corresponding ledger facts. Movement Details shows price snapshots, fill consumption cost, any override reason, and established movement fields. Project Cost Totals and Machine Cost Totals show total/priced/unpriced litres, fill count, consumption cost, and average cost per priced litre for the selected inclusive period. Excel provides separate Movement Details, Project Cost Totals, Machine Cost Totals, and supplier Payment Details worksheets.
...
- Priority: Must
- Acceptance criteria: Current balance is Unknown before a gauge and reconciles after one; cancelled movements are excluded from current totals; purchase spend and consumption cost remain separate; project/machine totals equal included active fills; unpriced litres are displayed separately rather than counted as zero-cost litres; Today, current month, and custom Excel date filters reconcile; no hour-meter calculation is present.
...
- Main behavior: Display read-only project/day operational links. Loads Delivered That Day contains matching confirmed plant loads and active quarry/supplier purchases in separate subsections. Fuel Used That Day contains every matching active equipment fill with time, equipment, litres, immutable price snapshot or Unpriced state, consumption cost, optional odometer reference, and notes. Show total litres, costed total, and unpriced litres separately in the app, PDF, and Excel.
...
- Priority: Must
- Acceptance criteria: Confirm a project load or fuel fill and verify it appears only in the matching project/date daily report. A cancelled fuel fill is excluded. Fill price and cost reconcile with the original fuel transaction, and unpriced fills remain visible without being counted as zero cost. Corrections made in the source workflow appear in later app views and exports without changing manual report content.
...
### DR-009
- A saved conversion option shall retain its case-insensitively unique name, rate, input unit, output unit, displayed decimal places, active state, and stable selection identity. Receipt Setup organizes conversions into alphabetic Active and Inactive groups with Edit, Remove, and Reactivate actions. Distinct names may share a rate. Conversion options are not associated with or filtered by item; every active option whose two units are active is available for manual receipt selection. A used option cannot be deleted and is deactivated by Remove; it may be edited for future use/reactivated. An unused option may be permanently removed. Confirmed records retain immutable conversion snapshots and results. (Sources: Turns 14, 19, 99, 119, 123, 303-304, and 434; status: Confirmed and implemented.)
...
### DR-012
- A quarry purchase record shall reference one saved quarry/supplier profile and may optionally reference one saved project; leaving project blank represents a general/non-project purchase. It shall include item, required positive whole cubic-metre quantity manually copied from the supplier ticket/invoice, delivery method (`company` or `supplier`), and automatic confirmation date/time. Company delivery requires saved driver and truck identities/snapshots. Supplier delivery requires neither company profile, snapshots `Supplier Delivering`, and may retain an optional free-text supplier truck/plate. Supplier delivery-ticket/invoice number and up to 20 readable-quality compressed paper-document photos are optional. Optional pricing retains USD price per cubic metre, calculated subtotal, applied universal VAT-rate snapshot, VAT amount, final total, derived paid amount, remaining balance, and status, and relates to zero or more separate payment entries under the customer-order payment/cancellation rules. An unpriced purchase has no balance. Attachments remain available offline, synchronize, and restore with the record. The supplier profile requires name and may retain phone, email, address, Tax/VAT number, and notes. (Sources: Turns 21, 26–29, 170–179, 200, 374, and 433–434; status: Confirmed for version-one entry fields.)
...
### DR-017
- A project daily work report shall be uniquely associated with one required saved project and one required non-future work date and retain its foreman/author, required short activity description, creation date/time, and latest-update date/time. It shall contain always-present, optionally empty sections for manual worker names, driver names, truck plates, machine names, material entries, notes, problems/delays/incidents, weather/site conditions, working time, next work planned, and up to 20 photo attachments. It presents a derived, read-only Loads Delivered That Day parent split into `DROMEX / Plant Loads` and `Quarry & Supplier Loads`, plus a separate Fuel Used That Day section containing every active same-project/date equipment fill with its immutable price/cost or explicit Unpriced state. Cancelled source records are excluded. Working time may retain start time, end time, and break duration. Each manually supplied material entry remains independent of linked operational records. Photos remain offline and synchronized. The report remains editable without approval or locking and retains no detailed edit log or prior versions. Its PDF includes linked loads, fuel fills, every other section, and all photos; its Excel export includes separate Linked Loads, Quarry Supplier Loads, Fuel Used, and Photos worksheets. (Sources: Turns 32–38, 194–203, 244, 292, 433–436; status: Confirmed and implemented through Turn 436.)
...
- Status: Confirmed and implemented in code through Turn 416; controlled consumption-entry correction, optional covered area, saved purposes, and Daily Report inclusion implemented under DEC-450 to DEC-454 (see "Wall Construction Area, Purposes, Corrections, and Daily Report Update"); physical mobile acceptance remains pending
...
## Supplier Loads, Safety, and Report Privacy Update

- The Suppliers workflow shall record any externally supplied catalog item with a snapshotted active measurement unit.
- New supplier references shall use `SUP`; legacy `QP` references and all existing relationships shall remain unchanged.
- Pricing shall present Per Unit and As a Whole choices, keep the amount field labelled Price, permit Unpriced records, and apply whole price once per separately incremented delivery.
- VAT shall support included/excluded treatment and company, zero, or custom rate with an immutable transaction snapshot.
- Reasoned supplier-load correction shall retain field-level original/new audit history and recalculate payment balance; active payments prevent moving the record to another supplier or making it Unpriced.
- Load History shall separate Company Loads and Supplier Loads under one entry point.
...
- Daily Report PDF shall support an optional Ministry header, configured globally in Company Settings as a Ministry name and Ministry logo and enabled per report by a `Show Ministry Header` option that defaults OFF (DEC-389). When the option is OFF the PDF shall contain no ministry name or logo and the page-one header shall keep its existing unbranded layout with the report title in its current right-hand position. When it is ON the ministry name and logo shall appear on page one only and never on a later page, as the right-hand identity of the header row with the company name and logo on the left, the report title moving to a strip directly beneath that header row; if only one value is configured the configured part alone shall appear; if neither is configured the app shall warn, naming what is missing, and the PDF shall omit the ministry block entirely. The PDF shall use the ministry values configured at generation time, so changing the global configuration changes regenerated historical PDFs.
- The Ministry logo shall retain its own intrinsic colours as a logo-only exception to the shared orange/navy/cream document hierarchy, and shall introduce no additional application palette. The Ministry block is optional branding that identifies the authority the report is produced for and shall neither replace nor expand the `DAILY PROJECT REPORT` page title (DEC-262 as corrected).
- Daily Report PDF shall split optional Consultant Sign-off output across two placements (DEC-390). **The page-one half of this rule is superseded by DEC-399 and DEC-401**: the Consulting Agency header is now governed by its own independent per-report option and is composed in the institutional header block, not beneath the report title, and it no longer depends on Consultant Sign-off. The clause as originally confirmed read: when the sign-off is ON and a Consulting Agency Name is configured, that agency name alone shall appear on page one directly beneath the report title as an organisational header, never the consultant's personal name; the consultant's personal name and digital signature shall appear together at the end of page two, immediately before the Photo Evidence section. When the sign-off is OFF, neither placement shall render an agency name, a personal name, or a signature. Every behaviour confirmed by DEC-388 is retained: optional and off by default, optional signature, no data deletion when disabled, the explicit `Consultant sign-off incomplete` state naming what is missing, and no signature image while incomplete.
- Company Settings shall hold one optional global Consulting Agency Name (DEC-391), shared by every project and permitted to be empty. **Ownership of this value moves to the PDF Settings screen under DEC-397, and an Arabic name is added under DEC-398.** It shall never affect sign-off completeness: `consultantSignoffState` continues to depend only on the consultant's personal name and signature. ~~A PDF shall use the agency name configured at generation time, so changing it changes regenerated historical PDFs.~~ **This clause is superseded by DEC-417 and DEC-416**: the single global agency becomes a reusable, per-project list of saved agencies (the underlying `company_settings` columns are kept, unused, rather than dropped), and a Daily Report now captures the agency's English and Arabic names as a historical snapshot at the moment it is selected, so a later rename, deactivation, or project reassignment no longer changes a regenerated historical PDF. Ministry and Custom Header are unaffected and remain live-read exactly as this clause originally described.
- Existing non-ministry Daily Report PDFs shall remain visually unchanged apart from the corrected consultant placement above.
- Ministry, Consulting Agency, and Consultant output shall never imply approval, certification, or government endorsement. Permitted field labels are `Consultant name` and `Signature`; the agency name shall carry no verb; wording such as `Approved by` or `Certified by` is prohibited, consistent with the absence of any report approval workflow (DEC-032).
...
## PDF Settings, Bilingual Document Headers, and Project Information Update

- A dedicated **PDF Settings** screen shall exist under the More tab's Setup group and shall own the Ministry name in English and Arabic, the Ministry logo, the Consulting Agency name in English and Arabic, and the Custom Header text in English and Arabic (DEC-397). Company & VAT shall keep company identity, company logo, contact details, Tax/VAT registration number, receipt footer message, VAT rate, and DEMO record visibility. No moved control shall appear on both screens.
- The Daily Report PDF shall support three independent optional institutional headers, Ministry, Consulting Agency, and Custom Header, each with global bilingual values and its own per-report option, and any combination of the three shall be permitted (DEC-398). A new report shall default all three options to Off, and switching an option off shall never delete that header's stored global values.
- The Consulting Agency header shall be independent of Consultant Sign-off in both directions (DEC-399). The migration adding `show_consulting_agency` shall **backfill** it to on for every existing report whose Consultant Sign-off is enabled, so reports that print an agency line today continue to print it when regenerated; every other existing report and every new report shall default to off.
- English header text shall render left-to-right on the left and Arabic shall render right-to-left on the right with `dir="rtl"` and `lang="ar"`, facing each other (DEC-400). When only one language is configured the layout shall stay balanced with no empty facing column. Arabic shall use a system font stack with no bundled font asset, shall be correctly shaped and ordered in the produced PDF, and long values in either language shall wrap without overlapping a logo, another header line, or the divider.
- Page one shall be composed as a logo row (company left, Ministry right when enabled), then the institutional header text block in the order Ministry, Consulting Agency, Custom Header, then a restrained divider, then the centred Daily Project Report title (DEC-401). Logos shall scale proportionally and shall never be stretched, cropped, or recoloured. Switching Ministry off shall remove the Ministry logo and name only and shall never remove the company logo.
- The Daily Report editor shall expose all three options in one collapsible `PDF Headers` section replacing the existing `Ministry Header` section, keeping the editor at twelve sections (DEC-402). Each control shall state which of its global English, Arabic, and logo values are configured and shall offer a direct Open PDF Settings action when configuration is missing.
- Consulting agencies shall be saved once and reused across projects, replacing the single global name and Arabic name pair (DEC-417). PDF Settings' existing section 02 shall become a manager offering add, edit, search, deactivate, and reactivate; an agency shall never be physically deleted. A newly created agency shall require a non-empty English name; an Arabic name is optional. Duplicate English names shall be rejected by the database on a normalized key (trimmed, internal whitespace collapsed, case-folded), computed and written only by the repository. Each project shall carry an optional, live-read consulting-agency assignment, editable from the existing Edit Project Information workflow, offering every active agency plus the project's own currently assigned agency even if it has since been deactivated, and no other inactive agency. A new Daily Report shall inherit its project's current agency once, as a historical snapshot (DEC-416), at creation; the report's `Show Consulting Agency` switch shall still default to Off regardless. The PDF Headers section shall offer a deliberate report-level override, including an explicit "No consulting agency" choice, and shall state whether the current value was inherited from the project or overridden for this report. A forward-only migration shall seed one agency from any existing global value, preserving an Arabic-only legacy value with an empty English name rather than inventing or copying text into it, and shall backfill existing projects and only those existing reports whose header was already on.
...
## Wall Construction Area, Purposes, Corrections, and Daily Report Update
...
- A Wall Construction consumption record for **Stone** or **Ready Mix** shall accept an optional covered wall area calculated as gross area (length × height) less openings/deductions in m², using the same face-area formula as the wall volume calculator (DEC-450). Length and height shall be greater than zero and openings zero or more, each with at most three decimals; NaN, infinity, and openings equal to or exceeding the gross area shall be rejected. The record shall store length, height, openings, gross area, and net area as a snapshot.
- Covered area shall never replace or change the consumed quantity or unit. A record without area shall display **Area not recorded**, never `0 m²`, and covered area shall not be offered for rebar or site-mixed records.
