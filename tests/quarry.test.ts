...
describe('quarry purchases',()=>{
  it('calculates subtotal, universal VAT, and final total',()=>{
    ...
    expect(result).toEqual({subtotalUsd:200,vatRatePercent:11,vatAmountUsd:22,finalTotalUsd:222});
  });
  it('leaves VAT empty for quantity-only purchases',()=>{
    expect(calculateQuarryPurchase({...emptyQuarryPurchaseDraft,quantityCubicMetres:'10'},11)).toEqual({subtotalUsd:null,vatRatePercent:null,vatAmountUsd:null,finalTotalUsd:null});
  });
  it('requires saved profiles, a unit, and a positive quantity',()=>{
    ...
    expect(validateQuarryPurchase({...valid,quantityCubicMetres:'12.5'},setup)).toEqual([]);
  });
  it('allows supplier delivery without a company driver or truck and keeps the supplier plate optional',()=>{
    const supplierDelivery={...emptyQuarryPurchaseDraft,supplierId:'s1',itemId:'i1',quantityCubicMetres:'20',deliveryMethod:'supplier' as const};
    expect(validateQuarryPurchase(supplierDelivery,setup)).toEqual([]);
    expect(validateQuarryPurchase({...supplierDelivery,supplierTruckPlate:'SUP-77'},setup)).toEqual([]);
  });
  it('rejects stale project references and excessive price decimals',()=>{const valid={...emptyQuarryPurchaseDraft,supplierId:'s1',itemId:'i1',quantityCubicMetres:'12',driverId:'d1',truckId:'t1'};expect(validateQuarryPurchase({...valid,projectId:'missing'},setup)).toContain('Select a valid project or clear the project field.');expect(validateQuarryPurchase({...valid,unitPriceUsd:'10.999'},setup)).toContain('Price must be zero or more with no more than two decimals.');});
  it('supports whole-delivery prices and VAT-inclusive quotes',()=>{expect(calculateQuarryPurchase({...emptyQuarryPurchaseDraft,quantityCubicMetres:'20',unitPriceUsd:'111',priceBasis:'whole',vatInclusive:true},11)).toEqual({subtotalUsd:100,vatRatePercent:11,vatAmountUsd:11,finalTotalUsd:111});});
  ...
  it('builds separate daily counters per project, supplier, item, driver, and truck',()=>{
    const purchase=(id:string,time:string,truckId:string,truckPlate:string,quantity:number,status:'Active'|'Cancelled'='Active')=>({id,confirmedAt:`2026-08-27T${time}:00`,projectId:'p1',projectName:'Mountain Road',supplierId:'s1',supplierName:'Main Quarry',itemId:'i1',itemName:'Aggregate',deliveryMethod:'company',driverId:truckId==='t1'?'d1':'d2',driverName:truckId==='t1'?'Ali':'Hassan',truckId,truckPlate,quantityCubicMetres:quantity,status} as QuarryPurchase);
  ...
  it('groups supplier deliveries by optional supplier plate',()=>{const purchase=(id:string,time:string,plate:string)=>({id,confirmedAt:`2026-08-27T${time}:00`,projectId:'p1',projectName:'Mountain Road',supplierId:'s1',supplierName:'Main Quarry',itemId:'i1',itemName:'Aggregate',deliveryMethod:'supplier',driverId:'system',driverName:'Supplier Delivering',truckId:'system',truckPlate:plate,quantityCubicMetres:20,status:'Active'} as QuarryPurchase);const counters=quarryDailyCounters([purchase('one','08:00',''),purchase('two','09:00',''),purchase('three','10:00','SUP-1')],'2026-08-27');expect(counters).toHaveLength(2);expect(counters.find(value=>!value.truckPlate)).toMatchObject({deliveryMethod:'supplier',tripCount:2,totalQuantityCubicMetres:40});});
});
