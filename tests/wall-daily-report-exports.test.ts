...
const wallA:LinkedWallWork={wallId:'wall-a',wallName:'Retaining wall A',system:'rubble_masonry',purpose:'retaining',lengthM:20,heightM:4,bottomThicknessM:.8,topThicknessM:.4,netVolumeM3:48,plannedVolumeM3:48,layers,base:null,baseEvents:[],baseStatusAsOf:null,entries:[
...
const wallB:LinkedWallWork={wallId:'wall-b',wallName:'Boundary wall B',system:'reinforced_concrete',purpose:'boundary',lengthM:12,heightM:2.5,bottomThicknessM:.3,topThicknessM:.3,netVolumeM3:9,plannedVolumeM3:9.45,layers:[],base:null,baseEvents:[],baseStatusAsOf:null,entries:[
...
describe('Daily Report workbook: Wall Construction sheet',()=>{
  ...
  it('adds the wall sheets after Waste Dumps and before Photos',()=>{
    expect(sheets().map(sheet=>sheet.name)).toEqual(['Report Overview','Work Details','Presence','Worker Safety','Materials','Linked Loads','Supplier Loads','Fuel Used','Waste Dumps','Wall Construction','Wall Layers','Wall Bases','Photos']);
  });
