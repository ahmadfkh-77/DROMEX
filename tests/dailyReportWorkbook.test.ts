...
describe('daily report workbook',()=>{
  ...
  it('embeds photo media through a dedicated Photos worksheet drawing',()=>{const image={name:'Photo 1',bytes:new Uint8Array([137,80,78,71,13,10,26,10]),extension:'png' as const,row:2,column:0};const files=unzipSync(buildDailyReportWorkbook(report,project,loads,quarry,waste,fuel,company,[image]));expect(files['xl/media/image1.png']).toEqual(image.bytes);expect(strFromU8(files['xl/worksheets/sheet10.xml']!)).toContain('<drawing r:id="rId1"/>');expect(strFromU8(files['xl/drawings/drawing1.xml']!)).toContain('Photo 1');expect(strFromU8(files['xl/drawings/_rels/drawing1.xml.rels']!)).toContain('../media/image1.png');});
  it('localizes generated labels and direction without translating entered text',()=>{const files=unzipSync(buildDailyReportWorkbook(report,project,loads,quarry,waste,fuel,company,[],'ar')),workbook=strFromU8(files['xl/workbook.xml']!),work=strFromU8(files['xl/worksheets/sheet2.xml']!);expect(workbook).toContain('نظرة عامة على التقرير');expect(work).toContain('rightToLeft="1"');expect(work).toContain('صب خرسانة Concrete pour');expect(work).toContain('17:00');});
});
