...
describe('migration 43: fresh install', () => {
  it('reaches version 43 with an empty construction_lifts table and no foreign-key violations', async () => {
    const db = new TestDatabase(); databases.push(db);
    await migrateDatabase(db as never);
    expect(DATABASE_VERSION).toBeGreaterThanOrEqual(43);
...
describe('migration 43: schema constraints', () => {
  it('rejects a lift with both a foundation_id and a wall_id', async () => {
    ...
    expect(() => db.raw.exec(`
      INSERT INTO construction_lifts (id,parent_type,foundation_id,wall_id,sequence,reference,start_elevation_m,length_m,height_m,bottom_thickness_m,top_thickness_m,net_lift_volume_m3,created_at,updated_at)
  ...
  it('rejects a lift with neither a foundation_id nor a wall_id', async () => {
    ...
    expect(() => db.raw.exec(`
      INSERT INTO construction_lifts (id,parent_type,sequence,reference,start_elevation_m,length_m,height_m,bottom_thickness_m,top_thickness_m,net_lift_volume_m3,created_at,updated_at)
  ...
  it('rejects a foundation-typed lift pointed at a wall id', async () => {
    ...
    expect(() => db.raw.exec(`
      INSERT INTO construction_lifts (id,parent_type,wall_id,sequence,reference,start_elevation_m,length_m,height_m,bottom_thickness_m,top_thickness_m,net_lift_volume_m3,created_at,updated_at)
  ...
  it('rejects a duplicate sequence within the same foundation', async () => {
    ...
    db.raw.exec(`INSERT INTO construction_lifts (id,parent_type,foundation_id,sequence,reference,start_elevation_m,length_m,height_m,bottom_thickness_m,top_thickness_m,net_lift_volume_m3,created_at,updated_at) VALUES ('l1','foundation','${foundation.id}',1,'Lift 1',0,10,1,1,1,10,'${NOW}','${NOW}')`);
    expect(() => db.raw.exec(`INSERT INTO construction_lifts (id,parent_type,foundation_id,sequence,reference,start_elevation_m,length_m,height_m,bottom_thickness_m,top_thickness_m,net_lift_volume_m3,created_at,updated_at) VALUES ('l2','foundation','${foundation.id}',1,'Lift 2',0,10,1,1,1,10,'${NOW}','${NOW}')`)).toThrow();
  });
  it('allows the same sequence number reused across two different parents', async () => {
    const {db, foundation, wall} = await seededDatabase();
    db.raw.exec(`INSERT INTO construction_lifts (id,parent_type,foundation_id,sequence,reference,start_elevation_m,length_m,height_m,bottom_thickness_m,top_thickness_m,net_lift_volume_m3,created_at,updated_at) VALUES ('l1','foundation','${foundation.id}',1,'Lift 1',0,10,1,1,1,10,'${NOW}','${NOW}')`);
    expect(() => db.raw.exec(`INSERT INTO construction_lifts (id,parent_type,wall_id,sequence,reference,start_elevation_m,length_m,height_m,bottom_thickness_m,top_thickness_m,net_lift_volume_m3,created_at,updated_at) VALUES ('l2','wall','${wall.id}',1,'Lift 1',0,10,4,.8,.4,10,'${NOW}','${NOW}')`)).not.toThrow();
  });
  it('rejects an invalid status', async () => {
    const {db, foundation} = await seededDatabase();
    expect(() => db.raw.exec(`INSERT INTO construction_lifts (id,parent_type,foundation_id,sequence,reference,start_elevation_m,length_m,height_m,bottom_thickness_m,top_thickness_m,net_lift_volume_m3,status,created_at,updated_at) VALUES ('bad','foundation','${foundation.id}',1,'Lift 1',0,10,1,1,1,10,'inspected','${NOW}','${NOW}')`)).toThrow();
  });
...
describe('migration 43: legacy single-core data is untouched', () => {
  it('leaves foundation_composition_records and foundation single-core columns exactly as they were', async () => {
    ...
    expect(db.raw.prepare('SELECT COUNT(*) count FROM construction_lifts WHERE foundation_id=?').get(foundation.id)).toMatchObject({count: 0});
  });
});
