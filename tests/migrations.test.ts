...
import { DATABASE_VERSION, migrateDatabase, RESERVED_TEST_DATA_DEACTIVATION_SQL } from '../src/data/database/migrations';
...
describe('reserved test data deactivation migration', () => {
  ...
  it('is idempotent when re-run after already reaching the current version',async()=>{
    const statements:string[]=[];
    const db={execAsync:async(sql:string)=>{statements.push(sql);},getFirstAsync:async()=>({user_version:DATABASE_VERSION})};
    await migrateDatabase(db as never);
