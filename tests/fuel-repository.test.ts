...
describe('fuel price and fill-cost SQLite integration',()=>{
  ...
  it('keeps each fill price and cost unchanged after the current price changes',async()=>{const{database,repository}=await setup();await repository.setCurrentPrice({fuelType:'diesel',pricePerLitreUsd:'0.90',reason:'Initial price'});const first=await repository.recordFill({fuelType:'diesel',recordDate:'2026-08-29',equipmentType:'machine',equipmentId:'machine',projectId:'project',litres:'40',odometerReading:'',pricePerLitreUsd:'0.90',priceOverrideReason:'',notes:''});expect(first).toMatchObject({pricePerLitreUsd:.9,consumptionCostUsd:36,projectName:'Road Project'});await repository.setCurrentPrice({fuelType:'diesel',pricePerLitreUsd:'1.00',reason:'Supplier increase'});const history=(await repository.getSetup()).fuelPriceHistory;expect(history.map(value=>value.pricePerLitreUsd)).toEqual([1,.9]);const saved=(await repository.getOverview()).movements.find(value=>value.id===first.id);expect(saved).toMatchObject({pricePerLitreUsd:.9,consumptionCostUsd:36});const daily=await new SqliteProjectReportRepository(database as never).listLinkedFuelFills('project',localDateKey(first.confirmedAt));expect(daily[0]).toMatchObject({equipmentName:'Excavator',litres:40,pricePerLitreUsd:.9,consumptionCostUsd:36});});

  it('requires a reason when the owner overrides the current fill price',async()=>{const{repository}=await setup();await repository.setCurrentPrice({fuelType:'diesel',pricePerLitreUsd:'0.90',reason:''});await expect(repository.recordFill({fuelType:'diesel',recordDate:'2026-08-29',equipmentType:'machine',equipmentId:'machine',projectId:'project',litres:'10',odometerReading:'',pricePerLitreUsd:'1.00',priceOverrideReason:'',notes:''})).rejects.toThrow('reason for overriding');const fill=await repository.recordFill({fuelType:'diesel',recordDate:'2026-08-29',equipmentType:'machine',equipmentId:'machine',projectId:'project',litres:'10',odometerReading:'',pricePerLitreUsd:'1.00',priceOverrideReason:'Emergency purchase',notes:''});expect(fill).toMatchObject({consumptionCostUsd:10,priceOverrideReason:'Emergency purchase'});});

  it('can price a purchase and make it the new current price',async()=>{const{repository}=await setup();const purchase=await repository.recordDelivery({recordDate:'2026-08-29',supplierId:'supplier',litres:'1000',ticketNumber:'INV-1',pricePerLitreUsd:'0.95',updateCurrentPrice:true,notes:''});expect(purchase).toMatchObject({pricePerLitreUsd:.95,subtotalUsd:950});expect((await repository.getSetup()).currentFuelPrice?.pricePerLitreUsd).toBe(.95);});

  it('records a truck fill against the truck profile without using the machine foreign key',async()=>{const{database,repository}=await setup();database.raw.exec(`INSERT INTO truck_profiles (id,plate,make_model,is_active,created_at,updated_at) VALUES ('truck','T-445','Mercedes',1,'2026-08-29','2026-08-29')`);const fill=await repository.recordFill({fuelType:'diesel',recordDate:'2026-08-29',equipmentType:'truck',equipmentId:'truck',projectId:'project',litres:'75',odometerReading:'',pricePerLitreUsd:'',priceOverrideReason:'',notes:''});expect(fill).toMatchObject({equipmentType:'truck',equipmentId:'truck',equipmentName:'T-445',litres:75});expect(database.raw.prepare('SELECT equipment_type,equipment_id,truck_profile_id FROM fuel_movements WHERE id=?').get(fill.id)).toMatchObject({equipment_type:'truck',equipment_id:null,truck_profile_id:'truck'});});
});
