...
- **Domain visual identity — a third, bespoke instance.** Each of the ten sections carries a numbered Ledger-Cream tab (`01`–`10`), the same restrained-numbering idea Home and Project Command Center use, rebuilt locally for this screen rather than reused as shared code (extends the Numbered Marker Rule — see `.impeccable/design.json`'s narrative rules).
...
- **Section-entry summary**: a neutral "Sections with entries: N of 10" strip, computed from data already in memory — never called "completion," since nine of the ten sections are optional by design; only Work Information's badge switches to Status-Warning when empty, since it is the one genuinely required section.
