...
## Implemented on Saved Consulting Agencies (DEC-416/DEC-417)

`src/ui/screens/PdfSettingsScreen.tsx`'s section 02, `EditProjectInformationScreen.tsx`, and `ReportsScreen.tsx`'s `PDF Headers` section. No new visual language: every device this feature needed already existed and is reused as-is.

- **PDF Settings section 02** keeps its existing Ledger Folder shell (numbered cream tab, Structural Navy header, Signal Orange seam, Ledger Cream body) but its body becomes a manager instead of two free-text fields: an Add action, a search field once the list passes eight active agencies, active rows with Edit/Deactivate, and a Collapsible Status Band holding inactive agencies with Reactivate — the same device Projects and Customers use for their inactive-record groups. Add/Edit opens the **Focused Record Sheet** already established by Units & Conversions (`ReceiptSetupScreen.tsx`): a full-screen page-sheet modal with a required English field, an optional right-to-left Arabic field, and a duplicate-name error rewritten from the raw database constraint into a plain sentence, exactly as that screen already does for unit names.
- **Edit Project Information** gains one new field inside its existing card — a `SearchableSelect` for the project's consulting agency, with an explicit "No consulting agency" first choice rather than a blank row. It sits beside Notes, not in a second editor, and does not participate in the existing rename-confirmation dialog: assigning an agency is not an identity change.
- **PDF Headers' Consulting Agency control** keeps the shared `HeaderControl` shell (On/Off chips, English/Arabic Configuration State Pills) but its state now reads the report's own captured snapshot rather than a shared global value, and it adds one plain-language line stating whether the current value was **inherited from the project** or **overridden for this report**, plus a `SearchableSelect` offering every active agency, the project's own agency even if since deactivated, and "No consulting agency." A header switched on with nothing selected shows the same incomplete/warning treatment the other two headers already use when configured-but-unset.
- **Bilingual Document Header** is unchanged in the PDF itself: the agency line still faces English left and Arabic right exactly as DEC-400 describes; only the source of the two strings moved, from the global company row to the report's own snapshot.

No new component, colour, or motion was introduced. Everything above is an existing pattern applied to a new data shape.

## Implemented on Wall Construction consumption (DEC-450 to DEC-453)

`src/ui/screens/WallConstructionScreen.tsx` plus four focused components: `WallConsumptionForm`, `WallAreaCalculator`, `ConcretePurposeField`, and `WallConsumptionHistory`. A refinement inside the committed ledger identity, not a new visual language.

- **One form, two jobs.** `WallConsumptionForm` records a new consumption in section 2 and renders the correction form in place inside Consumption History, so both follow one field order: date, material, purpose, quantity, covered area, notes, then (when correcting) the reason. A failed save keeps every entered value and shows its error directly above the action that failed, announced through `accessibilityLiveRegion`, instead of only at the top of a long page.
- **Covered area is progressive disclosure in Calc Result Teal.** For Stone and Ready Mix only, a dashed teal "Add covered wall area" row opens a pale teal panel with length, height, and openings fields and a three-tile result strip directly beneath them: Gross area, Openings, and a solid-teal Net covered area tile. Teal is used because these are derived numbers (the Reserved-Hue Rule). The panel states that covered area never changes the consumed quantity. "Remove" clears only the area.
...
- **History rows open, they do not hide actions.** Each record shows its purpose or material, date, the shared quantity sentence, its covered area or the italic muted "Area not recorded", and a navy-outline "Corrected" tag when it has history. Tapping reveals details, the covered-area breakdown on a teal-soft block, and the correction trail newest first (`field: before → after`). **Correct This Record** is a secondary (navy outline) button, deliberately not the danger style, because correcting preserves the record. While correcting, the row's left stripe turns navy.
- **No zeros for missing data.** Metric cards above the wall list only totals that were actually recorded (plus Ready-mix and Stone area covered); a wall with no consumption shows a cream note instead of a grid of `0.00` cards. The shared quantity sentence omits missing site-mix ingredients.
...
- **Daily Report editor.** A read-only `08 Wall Construction` ledger section after Waste Dumps, with one teal-striped group per wall (the calculated-value accent, since wall rows are calculation records) and each record's material and purpose, quantity, and area or "Area not recorded". Later sections renumber to 09–13; the progress strip now counts eleven sections with entries.
- **Daily Report PDF.** `Wall construction that day` follows the waste dumps section: a cream wall header with a 2px Structural Navy seam carrying name, system, purpose, and geometry, then a four-column table (Material and purpose, Consumed quantity, Covered wall area, Notes). Text columns are left-aligned because quantity and area cells hold short sentences, not single figures. The correction note uses bold navy text, so every distinction survives grayscale. Rendered through the real print engine and inspected: headings stay with their tables, a short block moves whole to the next page, and a long table repeats its column headings.
