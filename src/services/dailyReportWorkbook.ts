...
import { type DailyProjectReport, type LinkedFuelFill, type LinkedFoundationActivity, type LinkedProjectLoad, type LinkedQuarryLoad, type LinkedWallWork, type LinkedWasteDump, type ProjectReportSetup, type ReportProject } from '../domain/projectReports';
...
export async function exportAndShareDailyReportWorkbook(report: DailyProjectReport, project: ReportProject, loads: LinkedProjectLoad[], quarry:LinkedQuarryLoad[], waste: LinkedWasteDump[], fuel:LinkedFuelFill[], company: ProjectReportSetup['company'], options: DailyReportWorkbookOptions = {}, wallWork: LinkedWallWork[] = [],
  /** DEC-470. Foundations with activity on this date but no wall linked yet. */
  foundationActivity: LinkedFoundationActivity[] = []) {
  ...
  try {
    ...
    const bytes = buildDailyReportWorkbook(report, project, loads, quarry, waste, fuel, company, images, locale, wallWork, foundationActivity); checkCancelled(options.signal);
