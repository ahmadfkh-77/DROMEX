...
import { type DailyProjectReport, type LinkedFuelFill, type LinkedProjectLoad, type LinkedQuarryLoad, type LinkedWallWork, type LinkedWasteDump, type ProjectReportSetup, type ReportProject } from '../domain/projectReports';
...
export async function exportAndShareDailyReportWorkbook(report: DailyProjectReport, project: ReportProject, loads: LinkedProjectLoad[], quarry:LinkedQuarryLoad[], waste: LinkedWasteDump[], fuel:LinkedFuelFill[], company: ProjectReportSetup['company'], options: DailyReportWorkbookOptions = {}, wallWork: LinkedWallWork[] = []) {
  ...
  try {
    ...
    const bytes = buildDailyReportWorkbook(report, project, loads, quarry, waste, fuel, company, images, locale, wallWork); checkCancelled(options.signal);
