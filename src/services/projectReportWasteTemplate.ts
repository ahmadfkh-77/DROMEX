...
import {customResourcesHtml,DAILY_REPORT_SECTIONS_CSS,supervisorSignoffHtml,supplierLoadsHtml} from './dailyReportSectionsTemplate';
...
export function buildProjectReportHtmlWithWaste(report:DailyProjectReport,project:ReportProject,loads:LinkedProjectLoad[],quarryOrWaste:LinkedQuarryLoad[]|LinkedWasteDump[],wasteOrCompany:LinkedWasteDump[]|ProjectReportSetup['company'],fuelOrLogo:LinkedFuelFill[]|string|null,companyOrPhotos:ProjectReportSetup['company']|(string|null)[],logo?:string|null,photosArg?:(string|null)[],includePricesArg=false,ministryLogoArg?:string|null,wallWorkArg:LinkedWallWork[]=[],foundationActivityArg:LinkedFoundationActivity[]=[]){
  ...
  const safetyRows=[...report.workers.map(name=>({name,type:'Worker',key:'worker'})),...report.drivers.map(name=>({name,type:'Truck Driver',key:'driver'})),...(report.operators??[]).map(name=>({name,type:'Operator',key:'operator'}))].map(person=>{const safety=(report.workerSafety??[]).find(value=>value.workerName===person.name&&(value.participantType??'worker')===person.key);const status=safety?.status==='compliant'?'Compliant':safety?.status==='missing'?'Missing PPE':'Not checked';const statusColor=safety?.status==='compliant'?'#287A55':safety?.status==='missing'?'#9A6512':'#65717D';const details=safety?.status==='missing'?(safety.missingItems.join(', ')||'Missing item not specified'):'—';return`<tr><td>${e(person.name)}</td><td>${e(person.type)}</td><td><span style="color:${statusColor};font-weight:700">${e(status)}</span></td><td>${e(details)}</td><td>${display(safety?.notes)}</td></tr>`;}).join('')||'<tr><td colspan="5" class="empty">No workers, truck drivers, or operators recorded</td></tr>';
  ...
  // DEC-478. Selected entries from Owner-defined directories, each under its own heading; omitted when none.
  const resourceBlocks=customResourcesHtml(report.customResources);
  const resourcesSection=resourceBlocks?`<section><h2 class="keep-next">Additional resources</h2>${resourceBlocks}</section>`:'';
...
${DAILY_REPORT_SECTIONS_CSS}
    ...
    <section class="section"><h2>People and equipment</h2><table><thead><tr><th>Section</th><th>Count</th><th>Present today</th></tr></thead><tbody><tr><td>Workers</td><td class="number">${report.workers.length}</td><td>${list(report.workers)}</td></tr><tr><td>Drivers</td><td class="number">${report.drivers.length}</td><td>${list(report.drivers)}</td></tr><tr><td>Operators</td><td class="number">${(report.operators??[]).length}</td><td>${list(report.operators??[])}</td></tr><tr><td>Trucks</td><td class="number">${report.truckPlates.length}</td><td>${list(report.truckPlates)}</td></tr><tr><td>Machines</td><td class="number">${report.machines.length}</td><td>${list(report.machines)}</td></tr></tbody></table></section>
    ${resourcesSection}
    ...
    <div class="page-two">
      ...
      ${supervisorSignoffHtml(report.supervisorSignoffs)}
