...
export function ReportsScreen({ repository,businessReportRepository,onBack,initialBusinessFilters,initialProjectId,initialReportId,startNewReport=false }: { repository: ProjectReportRepository;businessReportRepository:BusinessReportRepository;onBack: () => void;initialBusinessFilters?:Partial<BusinessReportFilters>;initialProjectId?:string|null;initialReportId?:string|null;startNewReport?:boolean }) {
  ...
  async function shareReportExcel(report: DailyProjectReport) {
    ...
    try { const [loads,waste] = await Promise.all([repository.listLinkedLoads(project.id, report.workDate),repository.listLinkedWasteDumps(project.id, report.workDate)]); await exportAndShareDailyReportWorkbook(report, project, loads, waste, setup.company,{locale:workbookLocale,signal:controller.signal,onProgress:setExportProgress}); setMessage(`Daily report Excel created in ${workbookLocale==='ar'?'Arabic RTL':'English LTR'}.`); }
  ...
  if (project) return (
    <ScrollView contentContainerStyle={styles.content}>
      ...
      {reports.length ? reports.map((report) => {const exportingThis=dailyExportingId===report.id;return <View key={report.id} style={styles.reportCard}><TouchableOpacity onPress={() => editReport(report)} disabled={busy}><View style={styles.reportCardHeader}><Text style={styles.reportCardTitle}>{report.workDate}</Text><Text style={styles.openText}>Open</Text></View><Text style={styles.reportDescription} numberOfLines={3}>{report.workDescription}</Text><Text style={styles.helper}>Updated {new Date(report.updatedAt).toLocaleString()} · {report.photos.length} photos</Text></TouchableOpacity>{exportingThis?<View style={styles.exportProgressTrack}><View style={[styles.exportProgressFill,{width:`${Math.max(2,exportProgress?.percent??2)}%`}]}/></View>:null}<View style={styles.reportExportRow}><TouchableOpacity style={[styles.exportButton,styles.reportExportButton]} disabled={busy} onPress={()=>void shareReport(report)}><Text style={styles.exportButtonText}>PDF</Text></TouchableOpacity><TouchableOpacity style={[styles.exportButton,styles.reportExportButton]} disabled={busy&&!exportingThis} onPress={()=>void shareReportExcel(report)}><Text style={exportingThis?styles.cancelExport:styles.exportButtonText}>{exportingThis?`${exportProgress?.percent??1}% · Tap to cancel`:'Excel + Photos'}</Text></TouchableOpacity></View>{exportingThis?<Text style={styles.helper}>{exportProgress?.message}</Text>:null}</View>}) : <View style={styles.empty}><Text style={styles.cardTitle}>No daily reports yet</Text><Text style={styles.helper}>Choose Make Daily Report to record the first workday.</Text></View>}
    </ScrollView>
  );
