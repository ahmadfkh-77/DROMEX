...
export function ReportsScreen({ repository, onBack }: { repository: ProjectReportRepository; onBack: () => void }) {
  ...
  return (
    <ScrollView contentContainerStyle={styles.content}>
      ...
      {active.length ? active.map((value) => <ProjectCard key={value.id} project={value} onPress={() => setProject(value)} />) : <View style={styles.empty}><Text style={styles.cardTitle}>No active projects</Text><Text style={styles.helper}>Add a project from Home → Projects first.</Text></View>}
      <Text style={styles.sectionTitle}>Completed Projects</Text>
      {completed.length ? completed.map((value) => <ProjectCard key={value.id} project={value} onPress={() => setProject(value)} />) : <Text style={styles.helper}>No completed projects.</Text>}
      <Text style={styles.sectionTitle}>Report generation</Text>
      <ExportOption title="Daily Project Report" description="Work, people, equipment, materials, delivered loads, waste dumps, notes, time, photos, and project context." format="PDF · Available now" ready />
      <ExportOption title="Completed Project Report" description="Start-to-finish totals, daily work timeline, materials, loads, waste dumps, time, issues, people, equipment, appendices, and photos." format="PDF · Available for completed projects" ready />
      <ExportOption title="Financial & Operational Reports" description="Loads and Sales, Customer Balances, Quarry Purchases, Supplier Balances, and payment details." format="Detailed generation coming in the reports slice" />
    </ScrollView>
  );
}
  ...
  return (
    <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
      ...
      <Section title="Work information">
        <Field label="Work date *" value={draft.workDate} onChangeText={(value) => update('workDate', value)} placeholder="YYYY-MM-DD" />
...
const styles = StyleSheet.create({
  ...
  reportCard: { backgroundColor: colors.surface, borderRadius: 15, padding: 16, gap: 5 }, businessCard: { backgroundColor: '#EEEAE2', borderRadius: 15, padding: 16, gap: 5 }, exportOption:{backgroundColor:colors.surface,borderRadius:15,padding:16,gap:7},ready:{color:colors.success,fontSize:10,fontWeight:'900',letterSpacing:1},planned:{color:colors.warning,fontSize:10,fontWeight:'900',letterSpacing:1},exportFormat:{color:colors.brandDark,fontSize:12,fontWeight:'900'}, empty: { borderWidth: 1, borderColor: colors.line, borderStyle: 'dashed', borderRadius: 15, padding: 16, gap: 5 },
  rowBetween: { flexDirection: 'row', justifyContent: 'space-between', gap: 10 }, openText: { color: colors.brandDark, fontWeight: '900' }, reportDescription: { color: colors.ink, fontSize: 14, lineHeight: 20 }, notice: { color: colors.warning, backgroundColor: '#FFF3D8', padding: 13, borderRadius: 11, lineHeight: 19 },
