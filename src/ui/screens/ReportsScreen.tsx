...
function DailyReportEditor({ setup, project, draft, linkedLoads, linkedQuarryLoads, linkedFuelFills, linkedWasteDumps, busy, error, onChange, onSave, onBack }: { setup: ProjectReportSetup; project: ReportProject; draft: DailyProjectReportDraft; linkedLoads: LinkedProjectLoad[]; linkedQuarryLoads:LinkedQuarryLoad[]; linkedFuelFills:LinkedFuelFill[]; linkedWasteDumps: LinkedWasteDump[]; busy: boolean; error: string | null; onChange: (draft: DailyProjectReportDraft) => void; onSave: () => void; onBack: () => void }) {
  ...
  return (
    <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
      ...
      <Section title="Worker safety / PPE" hint="Workers and truck drivers start as Not checked. Record compliance or missing safety equipment without blocking the report.">
        {safetyPeople.length?safetyPeople.map(person=>{const safety=(draft.workerSafety??[]).find(value=>value.workerName===person.name&&(value.participantType??'worker')===person.type)??{workerName:person.name,participantType:person.type,status:'not_checked' as const,missingItems:[],notes:''};return <View key={`${person.type}:${person.name}`} style={styles.safetyWorker}><Text style={styles.cardTitle}>{person.name} · {person.label}</Text><View style={styles.chips}><Choice label="Compliant" selected={safety.status==='compliant'} onPress={()=>updateSafety(person.type,person.name,{status:'compliant',missingItems:[]})}/><Choice label="Missing PPE" selected={safety.status==='missing'} onPress={()=>updateSafety(person.type,person.name,{status:'missing'})}/><Choice label="Not checked" selected={safety.status==='not_checked'} onPress={()=>updateSafety(person.type,person.name,{status:'not_checked',missingItems:[]})}/></View>{safety.status==='missing'?<><Text style={styles.label}>Missing equipment</Text><View style={styles.chips}>{safetyEquipment.map(item=><Choice key={item} label={item} selected={safety.missingItems.includes(item)} onPress={()=>updateSafety(person.type,person.name,{missingItems:safety.missingItems.includes(item)?safety.missingItems.filter(value=>value!==item):[...safety.missingItems,item]})}/>)}</View></>:null}<Field label="Safety notes" value={safety.notes} onChangeText={notes=>updateSafety(person.type,person.name,{notes})} placeholder="Optional observation"/></View>}):<Text style={styles.helper}>Add the present workers or drivers above first.</Text>}
      </Section>
