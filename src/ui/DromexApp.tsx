...
type Screen='home'|'projects'|'recordsHub'|'moreHub'|'projectCommand'|'search'|'drafts'|'attention'|'backup'|'cloud'|'printer'|'makeReceipt'|'loads'|'loadCorrections'|'receiptSetup'|'directory'|'customers'|'catalog'|'schedule'|'pavement'|'walls'|'reports'|'quarry'|'waste'|'fuel'|'quickText'|'financials'|'settings';
...
export function DromexApp(){
  ...
  else if(screen==='quarry')content=<QuarryPurchasesScreen repository={quarryRepository} initialProjectId={activeProject?.id} startEntry={entryIntent==='quarry'} initialPurchaseId={searchTarget?.kind==='Supplier Load'||searchTarget?.kind==='Quarry Purchase'?searchTarget.id:null} onBack={()=>goBack('recordsHub')}/>;
  ...
  const moreActive=(['moreHub','reports','catalog','receiptSetup','printer','settings','search','drafts','attention','backup','cloud'] as Screen[]).includes(screen);
