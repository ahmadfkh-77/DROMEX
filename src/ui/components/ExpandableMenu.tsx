...
export function ExpandableMenuSection({title,hint,tone='navy',marker,refined=false,polished=false,open,onToggle,children}:{title:string;hint:string;tone?:MenuTone;marker?:string;refined?:boolean;polished?:boolean;open:boolean;onToggle:()=>void;children:ReactNode}){
  ...
  return <View style={[styles.section,richHeader&&styles.sectionConnected]}>
    ...
    {render?<View style={[styles.body,refined&&styles.bodyRefined,polished&&styles.actionCards,richHeader&&styles.bodyConnected]}>{items.map((item,index)=>{const value=(polished?itemMotionRef.current[index]:undefined)??progress;return <Animated.View key={index} style={{opacity:value,transform:[{translateY:value.interpolate({inputRange:[0,1],outputRange:[-6,0]})}]}}>{item}</Animated.View>;})}</View>:null}
  </View>;
}
...
export function MenuAction({number,title,body,onPress,tone='white',badge,refined=false,polished=false}:{number:string;title:string;body:string;onPress:()=>void;tone?:'white'|'cream'|'orange';badge?:string;refined?:boolean;polished?:boolean}){
  ...
  // Project Command Center (`polished`): a separate bordered card per action, no ordinal number, a
  // readable description, and the arrow in a quiet circle.
  const content=<>
    {polished?null:<Text style={[styles.number,refined&&styles.numberRefined]}>{number}</Text>}
    <View style={styles.actionCopy}>
      ...
      <Text style={polished?styles.actionCardBody:styles.actionBody}>{body}</Text>
    </View>
    {polished?<View style={styles.actionCardArrow}><Text style={styles.actionCardArrowText}>›</Text></View>:<Text style={[styles.arrow,refined&&styles.arrowRefined]}>›</Text>}
  </>;
