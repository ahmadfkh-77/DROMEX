...
export const businessReportLabels:Record<BusinessReportKind,string>={loads:'Loads and Sales',customers:'Customer Balances and Payments',quarry:'Supplier Loads and Balances',fuel:'Fuel Movements and Current Balance',projects:'Projects and Daily Work Reports',analysis:'Complete Analysis Workbook'};
...
export function filterBusinessReportData(data:BusinessReportData,filters:BusinessReportFilters):BusinessReportData{
  ...
  const costTotals=(keyColumn:'Equipment ID'|'Project ID',nameColumn:'Equipment'|'Project')=>[...fuelMovements.reduce((totals,row)=>{if(row.Type!=='fill'||row.Status!=='Active')return totals;const key=String(row[keyColumn]??row[nameColumn]??'Unassigned'),current=totals.get(key)??{[keyColumn]:row[keyColumn]??null,[nameColumn]:row[nameColumn]??(nameColumn==='Project'?'No project linked':'Unknown equipment'),'Total Litres Filled':0,'Priced Litres':0,'Unpriced Litres':0,'Consumption Cost USD':0,'Fill Count':0};const litres=Number(row['Litres Out']??0),cost=row['Consumption Cost USD'];current['Total Litres Filled']=Number(current['Total Litres Filled'])+litres;current['Fill Count']=Number(current['Fill Count'])+1;if(cost==null)current['Unpriced Litres']=Number(current['Unpriced Litres'])+litres;else{current['Priced Litres']=Number(current['Priced Litres'])+litres;current['Consumption Cost USD']=Number(current['Consumption Cost USD'])+Number(cost);}totals.set(key,current);return totals;},new Map<string,WorkbookRow>()).values()].map(row=>({...row,'Average Cost / Priced Litre USD':Number(row['Priced Litres'])>0?Number(row['Consumption Cost USD'])/Number(row['Priced Litres']):null}));
  ...
  return{...data,loads,quarryPurchases,fuelMovements,equipmentTotals,projectFuelTotals,projects,dailyReports,materials,payments,openingBalances,customers,suppliers,activeFilters};
}
