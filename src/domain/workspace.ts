...
export type WorkspaceActivity={id:string;type:'Load'|'Daily Report'|'Waste Dump'|'Fuel'|'Quarry'|'Schedule'|'Issue'|'Photo';occurredAt:string;title:string;detail:string|null};
export type ProjectWorkspaceSnapshot={
  project:Project;
  metrics:{loads:number;netTonnes:number;dailyReports:number;wasteDumps:number;fuelLitres:number;quarryPurchases:number;scheduled:number;openIssues:number};
  activities:WorkspaceActivity[];
  issues:WorkspaceIssue[];
  photos:WorkspacePhoto[];
};

export type GlobalSearchRoute='loads'|'customers'|'projects'|'directory'|'catalog'|'reports'|'quarry'|'quickText'|'financials'|'waste'|'schedule';
