...
export type FuelDashboard={today:FuelPeriodTotals;month:FuelPeriodTotals;projects:FuelCostBreakdown[];equipment:FuelCostBreakdown[]};
...
function breakdown(fills:FuelMovement[],key:'project'|'equipment'):FuelCostBreakdown[]{const result=new Map<string,FuelCostBreakdown>();for(const fill of fills){const id=key==='project'?fill.projectId:fill.equipmentId,name=key==='project'?fill.projectName:fill.equipmentName,groupKey=id??`unassigned:${name??key}`;const current=result.get(groupKey)??{id:id??'',name:name??(key==='project'?'No project linked':'Unknown equipment'),litres:0,costUsd:0,unpricedLitres:0,fillCount:0};current.litres+=fill.litres;current.costUsd+=fill.consumptionCostUsd??0;if(fill.consumptionCostUsd==null)current.unpricedLitres+=fill.litres;current.fillCount+=1;result.set(groupKey,current);}return [...result.values()].sort((a,b)=>b.litres-a.litres||a.name.localeCompare(b.name));}
export function buildFuelDashboard(movements:FuelMovement[],now=new Date()):FuelDashboard{const today=localDateKey(now),month=today.slice(0,7),activeMonthFills=movements.filter(v=>v.status==='Active'&&v.type==='fill'&&localDateKey(v.confirmedAt).startsWith(month));return{today:periodTotals(movements,date=>date===today),month:periodTotals(movements,date=>date.startsWith(month)),projects:breakdown(activeMonthFills,'project'),equipment:breakdown(activeMonthFills,'equipment')};}
