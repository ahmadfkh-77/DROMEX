...
function seedDatabase(db:DatabaseSync,now:Date,loadCount:number){
  ...
  try{
    ...
    for(let index=0;index<20;index++)insert(db,'driver_profiles',{id:id('worker',index),name:`DEMO · Worker ${String(index+1).padStart(2,'0')}`,person_role:'worker',job_title:pick(['Foreman','Carpenter','Steel fixer','General worker','Surveyor'],index),phone:`+961 76 ${String(330000+index).padStart(6,'0')}`,notes:null,is_active:1,created_at:timestamp,updated_at:timestamp});
...
function collectCounts(databasePath:string){const tables=['loads','projects','customers','daily_project_reports','quarry_purchases','waste_dumps','fuel_movements','payment_entries','schedule_tasks','pavement_calculations','walls','wall_consumptions','project_issues','project_media','quick_text_documents','suppliers','driver_profiles','truck_profiles','machine_profiles','catalog_items','custom_directories','custom_directory_entries','supervisors'];const db=new DatabaseSync(databasePath,{readOnly:true});try{const counts:Record<string,number>={};for(const table of tables)counts[table]=Number((db.prepare(`SELECT COUNT(*) count FROM ${table}`).get() as{count:number|bigint}).count);return counts;}finally{db.close();}}
